...
A named queue may be a virtual queue, as described in RFC 33, which
schedules its jobs as part of a parent queue while allowing independent
submission and scheduling control. :program:`flux queue enable`,
:program:`flux queue disable`, :program:`flux queue start`, and
:program:`flux queue stop` operating on a virtual queue affect only that
queue's own state, never its parent's or any sibling virtual queue's.
