...
static int output_client_write (struct output_client *client,
                                int rank,
                                const char *stream,
                                const char *data,
                                int len,
                                bool eof)
{
    ...
    rc = output_client_send (client, "data", o);
    json_decref (o);
    return rc;
}

int output_client_handler (flux_plugin_t *p,
                           const char *topic,
                           ...
                           void *arg)
{
    ...
    if (flux_plugin_arg_unpack (args, FLUX_PLUGIN_ARG_IN,
                                ...
                                "rank", &rank,
                                "data", &data, &len) < 0) {
        shell_log_errno ("flux_plugin_arg_unpack: %s",
                         flux_plugin_arg_strerror (args));
        return -1;
    }

    return output_client_write (client,
                                rank,
                                stream,
                                data,
                                len,
