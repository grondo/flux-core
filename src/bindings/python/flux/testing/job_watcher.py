...
class PerJobEventWatcher(JobEventWatcher):
    ...
    def stop(self):
        # Per-job futures complete naturally when each job's eventlog
        # ends. Callers that need to terminate watching while jobs
        # are still active should arrange for those jobs to reach a
        # terminal state first (e.g. via cancel).
        #
        # TODO: actively cancel pending event-watch RPCs by sending
        # job-info.event-watch-cancel with each future's matchtag,
        # so stop() preempts in-flight watches when jobs are still
        # active.
        pass
