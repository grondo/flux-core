...
class QueueResources:
    ...
    def __init__(self, name, resources, config, parent=""):
        ...
        # instead of the full instance resource set.
        requires = None
        queues = config.get("queues", {})
        if name and name in queues:
            requires = queues[name].get("requires")
        if requires is None and parent and parent in queues:
...
class QueueInfo:
    ...
    Attrs:
        ...
        blocked (bool): True if this queue's own started bit is set but
            it is effectively stopped by an external condition (scheduler
            offline, or a virtual queue's parent stopped)
    ...
    def _config_entries(self):
        ...
        key still inherits its parent's other keys.
        """
