...
class QueueInfoWrapper:
    def __init__(self, queue_info):
        ...
        self.is_blocked = queue_info.blocked
    ...
    def is_paused(self):
        ...
        # effectively stopped because its parent queue is stopped. This
        # is the only "blocked" condition that gets a distinct display
        # (SCHED "stopped (parent)", yellow paused glyph): it is new with
        # virtual queues, so no prior expectation is perturbed, and it
        # clears on its own once the parent starts. Other blocked
        # conditions (e.g. scheduler offline) are pre-existing and remain
        # rendered as a plain red "stopped", unchanged from prior behavior.
        return self.is_blocked and bool(self.parent)
