...
class TLSFlux:
    ...
    def __new__(cls):
        if not hasattr(cls.tls, "_handle"):
            cls.tls.handle = flux.Flux()
        return cls.tls.handle
...
class CodeTask(Task):
    ...
    def _do_exec(self):
        conf = self.broker_config
        attrs = self.broker_attrs
        try:
            exec(self.code)
    ...
    def run(self):
        self.proc = Thread(target=self._do_exec)
        self.proc.start()
...
class ModuleLoadTask(Task):
    ...
    def run(self):
        self.rpc = self.handle.rpc(
            "module.load", {"path": self.name, "args": self.args}
        )
...
class ModuleRemoveTask(ModuleLoadTask):
    ...
    def run(self):
        self.rpc = self.handle.rpc("module.remove", {"name": self.name})
...
class ShellTask(Task):
    ...
    def run(self):
        self.proc = Popen(["bash", "-c", self.command])
...
class Modprobe:
    def __init__(self, conf, remove=False, handle=None):
        if handle is None:
            handle = TLSFlux()
        self.handle = handle
    ...
    def solve_all(self, runlevel=None):
        ...
        alternatives = [l.pop() for l in self.alternatives.values()]
    ...
    def solve(self, tasks=None, runlevel=None):
        ...
        all_tasks = ChainMap(self.modules, self.rc_tasks(), alternatives)
    ...
    def get_batch_list(self):
        ...
        while self.sorter.is_active():
            ...
            batches.append([self.tasks[x].name for x in ready])
...
def read_config(path="modprobe.toml"):
    path = Path(TLSFlux().attr_get("broker.rc1_path")).parent / "modprobe.toml"
    with open(path) as fp:
        return tomllib.load(fp)


def rc1(args):
    Modprobe(read_config()).run(runlevel=1)


def rc3(args):
    Modprobe(read_config(), remove=True).run(runlevel=3)


def show_tasks(args):
    remove = args.rc == 3
    M = Modprobe(read_config(), remove=remove)
