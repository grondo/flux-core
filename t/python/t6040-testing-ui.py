...
Pure-Python tests: the emitter writes to a :class:`io.StringIO`
rather than a real terminal, so no broker is required and the
rendered output can be inspected directly.

Tests assert on substring presence rather than exact byte layouts
so future column-width tuning doesn't break the suite. ANSI
escape sequences are stripped from the captured output before
substring checks unless the test specifically verifies color
behavior.
"""
...
# Required to set PYTHONPATH for the in-tree layout sharness
# runs us from. We don't call rerun_under_flux (these tests
# don't need a broker), but the import itself must still run.
...
# Strip ANSI CSI sequences from a string. Used to verify rendered
# content without depending on color/cursor escape bytes.
...
def _last_frame(stream):
    ...
    by cursor-up + erase-to-end-of-screen sequences) and return
    the last one with ANSI codes stripped. This is what the user
    would see at the end of the run."""
    raw = stream.getvalue()
...
class TestColorSupported(unittest.TestCase):
    ...
    def setUp(self):
        # Snapshot env so per-test mutations don't leak. The
        # _color_supported function reads NO_COLOR and TERM.
...
class TestProgress(_BaseEmitterTest):
    ...
    def test_progress_updates_count(self):
        ...
        # Force a final render (default throttle would skip the
    ...
    def test_progress_ignored_below_verbosity(self):
        """At NORMAL verbosity, progress events are dropped (the
        UI is informationally degraded but still renders stage
        transitions)."""
        e = TerminalEmitter(
            verbosity=NORMAL,
            stream=self.stream,
        ...
        frame = _last_frame(self.stream)
        # Count should not appear since the progress event was
        # below threshold.
...
class TestCompletion(_BaseEmitterTest):
    ...
    def test_metrics_rendered_on_complete(self):
        ...
        # Values formatted with adaptive rate precision: 832.5
        # is in the 100-999 range so it gets 1 decimal place.
    ...
    def test_raw_timestamps_filtered_out(self):
        ...
        UI shows only curated derived metrics. The raw timestamps
        are still preserved in the results JSON file for
        analysis — just not in the on-screen summary."""
        self.emitter.test_start(
            "throughput",
            stages=["execute"],
            config={},
        ...
        # The raw timestamp labels do not appear — neither the
        # key names nor the unformatted epoch values.
    ...
    def test_adaptive_rate_precision(self):
        ...
        no decimals, ≥100 one decimal, otherwise two decimals.
        Keeps the column readable across HPC-scale ranges."""
        self.emitter.test_start(
            "throughput",
            stages=["execute"],
            config={},
    ...
    def test_active_stage_finalized_to_done(self):
        ...
        (no explicit final progress event reaching total), the
        stage transitions to done so the bar appears full."""
        self.emitter.test_start(
            "throughput",
            stages=["execute"],
            config={},
...
class TestColorAndAscii(unittest.TestCase):
    ...
    def test_color_never_omits_escape_codes(self):
        ...
        # Cursor-control sequences ARE present (they're how the
        # block redraws); colors are not. Filter to SGR (style)
        # sequences which end in 'm'.
        raw = stream.getvalue()
...
class TestRedrawThrottling(unittest.TestCase):
    ...
    every call — the cost would dominate at scale (thousands of
    job events). A 50ms throttle is sufficient."""
    ...
    def test_unforced_redraws_throttle(self):
        ...
        e.test_start("throughput", stages=["execute"], config={})
        e.stage("execute", 0, 1)
        # Reset render timestamp so the very first throttled
        # progress call lands in the throttle window.
        e._last_render_t = e._last_render_t or 0
        # Hammer with 50 progress events; they should not all
        # produce frames (the throttle kicks in for unforced
        # redraws).
        before = stream.getvalue().count("execute")
        for i in range(50):
            e.progress(i, 100, "job")
        after = stream.getvalue().count("execute")
        # Each frame contains one "execute" occurrence. We expect
        # FAR fewer than 50 frames given the 50ms throttle and
        # the negligible cost of the loop.
        self.assertLess(after - before, 50)


if __name__ == "__main__":
    # Emit TAP output so sharness can parse per-test pass/fail
    # results. Unlike t6000/t6010/t6030, this file's tests are
    # pure-Python (no broker required) so rerun_under_flux is
    # not needed.
