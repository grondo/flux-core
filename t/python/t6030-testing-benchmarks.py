...
# Private helpers (underscore-prefixed) exercised directly by
# the pure-Python unit tests below without needing a broker.
...
def _silent_emitter():
    ...
    Even QUIET still emits test.start / result / test.complete to
    stdout, which would interleave with TAP output. Use an explicit
...
class _StageRecorder:
    ...
    Used in the benchmark "run invariants" tests to verify the
    benchmark actually traversed every expected stage. Methods are
    intentionally minimal — benchmarks only call stage() and
    progress() on the emitter, so we record those and ignore the
    rest. If a benchmark grows new emitter call sites, this class
    will raise AttributeError, which is the desired feedback.
    """
...
class TestSimpleJobspec(unittest.TestCase):
    ...
    def test_mock_false_omits_mock_execution(self):
        ...
        This is the real-execution code path: the broker
        actually runs the jobspec's command rather than
        simulating it.
        """
        spec = simple_jobspec(mock=False)
        # The "system.exec.test" subtree should not exist when
        # mock is False. Either the key is absent or the path
        # is incomplete.
    ...
    def test_mock_false_uses_supplied_command(self):
        ...
        (mock=True paths don't use the command, so the broker
        only honors it in real mode)."""
...
class TestTracker(unittest.TestCase):
    """Unit tests for the internal _Tracker helper that drives
    per-event progress emission in both benchmarks. Pure Python
    (no broker required) — the tracker only depends on having an
    object with a ``progress(current, total, label)`` method."""
    ...
    def test_gate_suppresses_emit_but_counts(self):
        ...
        opening it doesn't retroactively emit — call flush() for
        that. Subsequent steps with the gate open emit normally."""
        e = self._Emitter()
        open_gate = [False]
        t = _Tracker(250, "job", gate=lambda: open_gate[0])
        for _ in range(150):
            t.step(e)
    ...
    def test_flush_emits_current_count(self):
        ...
        stepping — used at phase boundaries to surface state
        accumulated while the gate was closed."""
        e = self._Emitter()
        t = _Tracker(250, "job")
        # Step the counter to 150 by stepping under a closed gate
        # so no events fire yet.
        open_gate = [False]
    ...
    def test_flush_with_zero_count_is_noop(self):
        ...
        Used by ThroughputBenchmark.on_all_submitted: in the
        common case no clean events fired during submit phase
        and the flush should be silent."""
        e = self._Emitter()
        t = _Tracker(100, "job")
        t.flush(e)
        self.assertEqual(e.events, [])
    ...
    def test_label_passes_through(self):
        ...
        distinguish 'job' progress from 'cancel' progress in
        fill-machine."""
        e = self._Emitter()
        t = _Tracker(100, "cancel")
        for _ in range(100):
            t.step(e)
...
class TestBenchmarkMetadata(unittest.TestCase):
    ...
    def test_benchmarks_registry_has_both(self):
...
class TestReportMetadata(unittest.TestCase):
    ...
    metadata used by ``flux schedbench report``. The class-level
    metadata is what makes per-test reporting work — these tests
    exist to catch drift when REPORT_HEADINGS or REPORT_FORMATS
    are edited (a typo in either would otherwise silently
    misformat or break at runtime). For each benchmark class we
    check that:

    1. Every field referenced in every REPORT_FORMATS entry is
       declared in REPORT_HEADINGS (catches typos in format
       strings, removed-field drift).
    2. REPORT_HEADINGS contains all the identity columns from
       _COMMON_REPORT_HEADINGS (catches accidental shadowing in
       per-benchmark dicts).
    3. A ``default`` format exists (the CLI default).
    4. Each REPORT_FORMATS entry has both ``description`` and
       ``format`` keys (the shape UtilConfig expects).
    """

    #: All benchmark classes to test (both registered and any
    #: future additions will be covered automatically).
    BENCH_CLASSES = (ThroughputBenchmark, FillMachineBenchmark)

    def _format_fields(self, fmt_str):
        """Extract field names from a format string the same way
        ``OutputFormat`` does. Returns a list (not a set) so we
        can assert on the order the fields appear."""
        from string import Formatter

        names = []
        for _, field_name, _, _ in Formatter().parse(fmt_str):
            if field_name:
                # Strip any "!conv" suffix and ".attr" lookups;
                # schedbench formats don't use either today, but
                # this matches OutputFormat's behavior.
    ...
    def test_every_format_field_in_headings(self):
        ...
        drift-prevention test: editing REPORT_FORMATS to use
        a field name that doesn't exist in headings would cause
        OutputFormat to raise ValueError("Unknown format field")
        at runtime, breaking ``flux schedbench report``."""
    ...
    def test_common_headings_inherited(self):
        ...
        common heading dict so identity columns (time, scheduler,
        nodes, ...) are spelled consistently across benchmarks."""
        from flux.testing.schedbench.benchmarks import (
            _COMMON_REPORT_HEADINGS,
        )

        for cls in self.BENCH_CLASSES:
            for key, value in _COMMON_REPORT_HEADINGS.items():
                self.assertIn(
                    key,
                    cls.REPORT_HEADINGS,
...
class TestThroughputBenchmark(_BaseTestCase):
    ...
    def _inject(self, **kw):
        """Inject fake R for the test; returns (handle, fake)."""
        h = flux.Flux()
    ...
    def test_throughput_watcher_swap_equivalent_results(self):
        ...
        well-ordered metric set with positive rates and the same
        njobs. They differ in script-side timing (watcher
        overhead), but every invariant the benchmark guarantees
        must hold for either choice. Catches a watcher-factory
        threading bug — if watcher_factory weren't being passed
        through to BulkRun, one of these would fail or hang."""
        for name, factory in (
            ...
            with self.subTest(watcher=name):
                ...
                self.assertEqual(m["njobs"], 20)
                # All headline rates must be finite and positive.
                # ingest_rate is the broker-side submit measure;
                # submit_rate is the script-observed counterpart.
                for key in (
                    "submit_rate",
                    "alloc_rate",
                    "ingest_rate",
                ...
                # Script-observed submit_rate cannot exceed the
                # broker-side ingest_rate within the same run:
                # the script's bulk callback fires only after the
                # watcher has delivered every submit event, so the
                # script's window is no shorter than the broker's.
    ...
    def test_throughput_run_invariants(self):
        """Stages emitted, jobs submitted+completed, metrics finite/positive, timestamps monotonically ordered, resources released"""
        h, fake = self._inject(nodes=4, cores_per_node=4)

        # Snapshot job counts before the benchmark so we can isolate
        # this run's submissions from any from prior tests (the broker
        # accumulates inactive jobs across the whole subflux session).
        ...
        # The benchmark advertises two stages ("submit" then
        # "execute") and must actually traverse both; otherwise
        # on_event never wired up or on_all_submitted didn't
        # fire.
        self.assertEqual(rec.stages, ["submit", "execute"])

        # Exactly 20 new jobs reached the broker and all transitioned
        # to inactive. Catches "njobs was reported correctly but fewer
        # jobs actually got submitted" silently.
        self.assertEqual(after.total - before.total, 20)
        self.assertEqual(after.inactive - before.inactive, 20)
        # Mock-exec jobs return 0, so all 20 should be counted as
        # successful — not failed/canceled/timeout.
        self.assertEqual(after.successful - before.successful, 20)
        self.assertEqual(after.failed - before.failed, 0)

        # Every rate metric must be finite and positive — catches
        # inf/nan from divide-by-zero on degenerate event spans.
        for key in (
            "submit_rate",
            "alloc_rate",
            "ingest_rate",
        ...
        # Watcher overhead invariant: submit_rate is the script-
        # observed rate (njobs / (t_all_submitted - t_submit_start));
        # ingest_rate is the broker-side counterpart (njobs /
        # (t_last_submit_event - t_submit_start)). The script's
        # bulk callback fires after the broker's last event is
        # delivered, so the script's denominator is at least as
        # long, hence submit_rate <= ingest_rate. A violation
        # would mean the bulk callback and broker event
        ...
        # The full timestamp set is present. Downstream tools rely
        # on the exact key names, so check them explicitly.
        for key in (
            "t_submit_start",
            "t_all_submitted",
        ...
        # Timestamp orderings, split into chains the implementation
        # actually guarantees. A single monotonic chain across
        # script and broker times isn't valid: flux pipelines
        # events, so a broker-side first-Y event can precede a
        # script-side all-X bulk-callback firing.

        # Script-side callback ordering (BulkRun fires bulk
        # callbacks sequentially in lifecycle order; bulk.run
        # returns after on_all_clean):
        ...
        # Cross-event-type broker ordering by per-job causality:
        # the job that has last_X also has X before its own Y,
        # and that Y ≤ last_Y over all jobs.
        self.assertLessEqual(
            m["t_last_submit_event"],
        ...
        # Watcher delivery latency: broker emits each event
        # before the script's matching bulk callback fires.
        self.assertLessEqual(
            m["t_last_submit_event"],
            m["t_all_submitted"],
        )
        ...
        # Submit starts in the script before the broker sees
        # any submit event.
        self.assertLessEqual(
            m["t_submit_start"],
            m["t_first_submit_event"],
        ...
        # Both throughput numerators are njobs. Denominators:
        # throughput uses last_clean_event - t_submit_start
        # (broker-side right edge, script-side left edge).
        # script_runtime measures from BulkRun construction to
        # bulk.run() return, which always wraps the broker
        # event window (last_clean_event is delivered to the
        # script before bulk.run() returns). So script_runtime
        # >= the throughput denominator, hence
        # script_throughput <= throughput. 1% slop for floats.
        self.assertLessEqual(
            m["script_throughput"],
            m["throughput"] * 1.01,
        )

        # All resources should be free again: jobs completed and the
        # scheduler released their slots.
...
class TestFillMachineBenchmark(_BaseTestCase):
    ...
    def test_fill_machine_njobs_matches_saturation_count(self):
        ...
        which is what callers — including flux-schedbench — get
        from saturation_count on the resource shape. This is the
        end-to-end check that the public saturation_count helper
        and FillMachineBenchmark's njobs argument compose to
        produce a fully-packed broker."""
        h, fake = self._inject(
            nodes=4,
            cores_per_node=4,
            gpus_per_node=2,
    ...
    def test_fill_machine_run_invariants(self):
        """All three stages emitted, jobs canceled, metrics finite/positive, timestamps monotonically ordered, resources released"""
        h, fake = self._inject(nodes=4, cores_per_node=4)
        ...
        # The benchmark must traverse submit → fill → cancel in
        # order. If "fill" is missing, on_all_submitted never
        # fired. If "cancel" is missing, on_all_started never
        # fired (i.e., not all jobs reached "start"). Either gap
        ...
        # mock_run_duration="0" so they only exit via cancel).
        # JobStats counts canceled jobs under "failed" as well.
        ...
        # Watcher overhead invariant: submit_rate (script-
        # observed) is bounded above by ingest_rate (broker-
        # side). See ThroughputBenchmark for the derivation.
        self.assertLessEqual(
            m["submit_rate"],
            m["ingest_rate"] * 1.001,
        )
        ...
        # Timestamp orderings. The previous version of this test
        # asserted a single 12-element monotonic chain spanning
        # script-side and broker-side timestamps, but flux
        # pipelines events: by the time BulkRun's on_all_alloc
        # fires (script-side, last job's alloc delivered), the
        # broker has often already emitted the first start event
        # for an earlier job. Pipelining breaks "all-of-phase-X
        # before any-of-phase-Y" assumptions whenever they cross
        # the script/broker boundary. The split below captures
        # only what the implementation actually guarantees.

        # (1) Script-side callback ordering. BulkRun fires the
        # bulk callbacks sequentially in lifecycle order; cancel
        # is issued from inside on_all_started; bulk.run()
        # returns after on_all_clean.
        self.assertLessEqual(m["t_submit_start"], m["t_all_submitted"])
        self.assertLessEqual(m["t_all_submitted"], m["t_all_alloc"])
        ...
        # (3) Cross-event-type broker ordering by per-job
        # causality. For each job, submit < alloc < start <
        # clean. The job that has the latest X event had its
        # X event before its own Y event (Y after X in
        # lifecycle), and its Y event ≤ the latest Y event
        # across all jobs. So last_X ≤ last_Y. This does NOT
        # extend to first_X vs last_Y across phases.
        self.assertLessEqual(
            m["t_last_submit_event"],
            m["t_last_alloc_event"],
        )
        ...
        # (4) Watcher delivery latency: the broker emits each
        # event before the script's bulk callback for that
        # event type fires (the watcher delivers events into
        # BulkRun, which then aggregates into the bulk
        # callback).
        self.assertLessEqual(
            m["t_last_submit_event"],
            m["t_all_submitted"],
        ...
        # (5) Submit starts in the script before the broker
        # observes any submit event.
        self.assertLessEqual(
            m["t_submit_start"],
            m["t_first_submit_event"],
        )

        # (6) Cancel-then-clean: cancel is issued from
        # on_all_started, so it precedes any subsequent clean
        # event in the broker.
        self.assertLessEqual(
            m["t_cancel_issued"],
            m["t_last_clean_event"],
        ...
        # After cancel-and-cleanup, all allocated resources should
        # be released back to free.
        rl = flux.resource.resource_list(h).get()
        self.assertEqual(rl.allocated.nnodes, 0)
...
class TestBenchmarkResults(unittest.TestCase):
    ...
    def setUp(self):
        # Each test gets its own temp directory so files are
        # isolated; tempfile.TemporaryDirectory cleans up on tearDown.
    ...
    def test_save_atomic_no_tmp_leak(self):
        ...
        results.add_run({"test_name": "throughput"})
        results.save()
        # The tmp file should have been renamed; only the final
        # path remains.
...
class _FakeEvent:
    ...
    just enough for _failure_diagnostic to read context."""
...
class TestFailureDiagnostic(unittest.TestCase):
    """_failure_diagnostic groups identical errors into idsets
    ...
    def test_uniform_failures_collapse_to_idset(self):
        ...
        msg = _failure_diagnostic(r, "throughput: all submits failed")
        # The 1024 submission indices collapse into a single
        # "0-1023:" prefix on a single line.
    ...
    def test_backward_compat_no_submit_index(self):
        ...
        from older BulkRun versions) still render — just as a
        count rather than an idset."""
        r = _FakeBulkRunResult(
            njobs=0,
            attempted=3,
            submit_failures=[
