...
test_expect_success 'flux sqlite query rejects multi-statement queries' '
	test_must_fail flux sqlite query "SELECT 1; SELECT 2" 2>multi.err &&
	grep -q "multiple SQL statements" multi.err
'
...
test_expect_success 'flux sqlite backup rejects overwriting existing file' '
	BACKUP=$(pwd)/backup.db &&
	test_must_fail flux sqlite backup $BACKUP 2>backup-exists.err &&
	grep -q "already exists" backup-exists.err
'
